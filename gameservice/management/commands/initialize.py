...
class Command(BaseCommand):
    def handle(self, **options):
        ...
        #pelit
        testipeli = Game(title="Testipeli", developer=testidevaaja, price=Decimal(15.5), url="http://localhost:8001")
        testipeli.save()
        testipeli.categories.add(rpg)
        testipeli.categories.add(singleplayer)
        testipeli.save()

        testipeli2 = Game(title="Testipeli2", developer=testidevaaja, price=Decimal(25.5), url="https://users.aalto.fi/~oseppala/game/example_game.html")
        testipeli2.save()
        ...
        payment = Payment(user=testikayttaja, price=testipeli.price, game=testipeli, pid="123", sid="123")
        payment.save()